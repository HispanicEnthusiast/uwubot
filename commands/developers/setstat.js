...
return
 ...
 if(args[0].toLowerCase()==="play"||args[0].toLowerCase()==="playin"||args[0].toLowerCase()==="playing") stat='PLAYING';
  if(args[0].toLowerCase()==="listen"||args[0].toLowerCase()==="listenin"||args[0].toLowerCase()==="listening") stat='LISTENING';
  if(args[0].toLowerCase()==="watch"||args[0].toLowerCase()==="watchin"||args[0].toLowerCase()==="watching") stat='WATCHING';



      client.user
    .setActivity(args.slice(1).join(" ").replace("$MEMBER$", totalMembers).replace("$GUILD$", totalGuilds), {
      type: stat,
      
    })
      message.channel.send(`<:hikariok:801419553841741904> | I set my ${stat.toLowerCase()} Status to :\n\`\`\`${args.slice(1).join(" ").replace("$MEMBER$", totalMembers).replace("$GUILD$", totalGuilds)}\`\`\``)
...
exports.info = {
  ...
  usage : "<\"WATCHING\"/\"STREAMING\"/\"LISTENING\"/\"PLAYING\"> (<twitch_username>) <status>",
