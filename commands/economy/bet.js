...
exports.run = async (client, message, args) => {
    ...
    if (amount < 1) return message.mentionReply("You don't have enough money for gambling. The minimum is 200 cookies.");
    ...
    if (result < 5) {
        ...
        return message.noMentionReply(`Oh noes, you lose ${amount} cookies. OwO`);
    } else if (result > 5) {
        await client.db.set(`lastGamble.${message.author.id}`, Date.now());
