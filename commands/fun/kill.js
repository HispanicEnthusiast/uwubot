exports.run = async (bot, message, args) => {
 ...
 let userm;
  let author;
  let usern;
  const fetch= require("node-fetch"),main = await fetch("https://hauntbot.glitch.me/api/kill.html"), mat = await main.json();
if (args[0]=== "me"||args[0]=== `<@!${message.author.id}>`) {
  ...
  if(!userm||!args[0]) {
return message.mentionReply('Remember to mention a valid user to kiss!');
}
