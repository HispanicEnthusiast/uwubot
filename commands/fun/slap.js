...
if (args[0]=== "me"||args[0]=== `<@!${message.author.id}>`) {
      ...
      userm = await message.guild.members.fetch(args[0].replace("<@!", "").replace("<@","").replace(">","")).catch(err => { return message.mentionReply("Remember to mention a valid user to slap!") })
      usern =userm.user
      //userm =use.user
  ...
  if(!userm||!args[0]) {
return message.mentionReply('Remember to mention a valid user to slap!');
}
