...
return message.mentionReply("No leaderboard availabe.");}
...
let data = client.db.all().filter(i => i.ID.startsWith("xp_")).sort((a, b) => b.data - a.data).splice(10 * (parseInt(args[1]) - 1), 10 * parseInt(args[1]));
      if(parseInt(args[1])> Math.ceil(Object.keys(data).length / 10)){client.sleep(2000);message.channel.stopTyping();return message.mentionReply("No leaderboard")}
    if (data.length < 1) {
      client.sleep(2000)
message.channel.stopTyping();
return message.mentionReply("No leaderboard available.");}
...
let data = client.db.all().filter(i => i.ID.startsWith("xp_")).sort((a, b) => b.data - a.data);
    if (data.length < 1) {
      client.sleep(2000)
message.channel.stopTyping();
return message.mentionReply("No leaderboard");
    }
    let myrank = data.map(m => m.ID).indexOf(`xp_${message.author.id}`) + 1 || "N/A";
    data.length = 10;
    let lb = [];
  ...
  if(args[0]){
    ...
    if (data.length < 1) {
      client.sleep(2000)
      message.channel.stopTyping();return message.mentionReply("No leaderboard");}
 ...
 let data = client.db.all().filter(i => i.ID.startsWith(`${ao}xp_`)).sort((a, b) => b.data - a.data).splice(10 * (parseInt(args[0]) - 1), 10 * parseInt(args[0]));
   if(parseInt(args[0])> Math.ceil(Object.keys(data).length / 10)){client.sleep(2000);message.channel.stopTyping();return message.mentionReply("No leaderboard!")}
      let data2 = client.db.all().filter(i => i.ID.startsWith(`${ao}xp_`)).sort((a, b) => b.data - a.data);
      if (data.length < 1) {client.sleep(2000);message.channel.stopTyping();return message.mentionReply("No leaderboard");}
