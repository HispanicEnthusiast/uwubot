...
exports.run = (bot, message, args) => {
  ...
  if(!permissions.has("BAN_MEMBERS")) return message.noMentionReply("<:koucry:801419554311241728> | <@!"+message.author.id+">, I don't have permission to ban!!!");
 if (!message.member.hasPermission("BAN_MEMBERS")&&!message.member.hasPermission("MANAGE_GUILD")&&!message.member.hasPermission("MANAGE_MEMBERS")&&!message.member.hasPermission("ADMINISTRATOR"))
        return message.mentionReply("<:tairitsuno:801419553933492245> | <@!"+message.author.id+">, You don't have permission to ban!!!");
...
const targe =  message.guild.member(target)
      ...
      if (targe.hasPermission("ADMINISTRATOR")/*hasPermission("ADMINISTRATOR")*/){
        return message.mentionReply("<:tairitsuno:801419553933492245> | <@!"+message.author.id+">,  The user you want to ban is a moderator/administrator I can't do that,try to ban him/her/them yourself..");
  }
