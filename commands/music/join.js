...
//checked
  run: async function (client, message, args) {
    ...
    if(message.guild.me.voice.channel !== channel)return sendError('<a:checkmark:854477462829006858> | The bot is already in an another channel, you need to join voice channel where the bot is to use this command!', message, client);
  }
    await channel.join();
    
    message.react("801419553841741904")
    sendSuccess("<a:checkmark:854477462829006858> | Joined successfully!", message);
  ...
  interaction: async function (client, message, args){//message=interaction
   ...
   let channel=await client.guilds.cache.get(message.guild_id).members.cache.get(message.member.user.id).voice.channel
    ...
    if (client.guilds.cache.get(message.guild_id).me.voice.channel !== channel)return sendError('<a:checkmark:854477462829006858> | The bot is already in another channel, you need to join voice channel where the bot is to use this command!', message, client);
  }
    await channel.join();
    //message.react("801419553841741904")
    sendSuccess("<a:checkmark:854477462829006858> | Joined successfully!", message, client);
