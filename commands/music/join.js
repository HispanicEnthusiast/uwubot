...
//checked
  run: async function (client, message, args) {
    ...
    if(message.guild.me.voice.channel !== channel)return sendError('<:Oops:858157163876319252> | I am already in an another voice channel, you need to join voice channel where I am in or move me!', message, client);
  }
    await channel.join();
    
    message.react("801419553841741904")
    sendSuccess("<a:Checkmark:858154320117235742> | Joined successfully!", message);
  ...
  interaction: async function (client, message, args){//message=interaction
   ...
   let channel=await client.guilds.cache.get(message.guild_id).members.cache.get(message.member.user.id).voice.channel
    ...
    if (client.guilds.cache.get(message.guild_id).me.voice.channel !== channel)return sendError('<:Oops:858157163876319252> | The bot is already in another channel, you need to join voice channel where the bot is to use this command!', message, client);
  }
    await channel.join();
    //message.react("801419553841741904")
    sendSuccess("<a:Checkmark:858154320117235742> | Joined successfully!", message, client);
