...
exports.run = async (bot, message, args) => {
  ...
  try {
    ...
    if (!body.data.length)
      return message.mentionReply("Can't find any results for that Anime TV Show.");
    ...
    console.log(err)
     message.mentionReply(
      `Error, please report it to Robert in the support server.`
    );
    return
  }
    ...
    if (!body.data.length)
      return bot.api.interactions(interaction.id, interaction.token).callback.post({
                data: {
                    type: 4,
                    data: {
                        content: "Can't find any results for that Anime TV Show."
                    }
                }
            });
                      const data = body.data[0].attributes;
