...
exports.run = (bot, message, args) => {
    let perm=message.channel.permissionsFor(message.member)//perm.has()
      ...
      if (!setup||isNaN(setup.replace("<@&", "").replace("<@", "").replace(">", "")))
        return message.mentionReply(
          `<:Oops:858157163876319252> | Please send a mute role id with this format ${bot.config.prefix}setmuterole [Mute Role ID]`
        );
      bot.db.set(`${message.guild.id}_muterole`, setup.replace("<@&", "").replace("<@", "").replace(">", ""))
      
      message.noMentionReply(
        `<a:Checkmark:858154320117235742> | Successfully setup mute role!`
      );
      return;
}
