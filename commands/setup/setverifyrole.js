...
if (!setup)
      ...
      if (!setup||isNaN(setup.replace("<@&", "").replace("<@", "").replace(">", "")))
        return message.mentionReply(
          `<:Oops:858157163876319252> | Please send a verify role id with this format ${bot.config.prefix}setverifyrole [verified role id]`
        );
      bot.db.set(`${message.guild.id}_verifyrole`, setup.replace("<@&", "").replace("<@", "").replace(">", ""))
      
      message.noMentionReply(
        `<a:Checkmark:858154320117235742> | Successfully setup verified role!`
      );
      return;
}
